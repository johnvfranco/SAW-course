...
class intr extends JFrame implements ActionListener {
   ...
   public intr (aFrame pnt) {
      super("Introduction to the course");
      ...
      try {
         fpath = (new java.io.File(".").getCanonicalPath())+"/src/intr/";
      ...
      rs.add(title = new JLabel("Introduction to the Course              ", JLabel.CENTER));
      ...
      p.setBackground(bkgd);
      p.add(lbl = new JLabel("  The purpose and goals of the course"));
      lbl.setFont(fnt);
      q.add(p);
      p = new JPanel(new FlowLayout(FlowLayout.LEFT));
      p.setBackground(bkgd);
      p.add(lbl = new JLabel("  Experiment with the tools"));
      lbl.setFont(fnt);
      q.add(p);
      xy.add("Center", q);
